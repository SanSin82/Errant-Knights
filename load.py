    ...
    if entry['event'] == 'Docked':
        ...
        if x >= 1:
            ...
            for i in range(0, z):
                ...
                inf = round(inf, 2)
                this.TodayData[x + 1][0]['Factions'].append({'Faction': this.FactionNames[i], 'INF': inf, 'State': 0, 'MissionPoints': 0, 'MissionFailed': 0,
            ...
            for i in range(0, z):
                inf = this.FactionStates['Factions'][i]['Influence'] * 100
                inf = round(inf, 2)
                this.TodayData[1][0]['Factions'].append({'Faction': this.FactionNames[i], 'INF': inf, 'State': 0, 'MissionPoints': 0, 'MissionFailed': 0,
                                                         'TradeProfit': 0, 'Bounties': 0, 'Bonds': 0, 'CartData': 0, 'Murders': 0})
    ...
    if entry['event'] == 'Location':  # get factions at startup
        ...
        for i in entry['Factions']:
            if i['Name'] != "Pilots' Federation Local Branch":
                ...
                this.FactionStates['Factions'].append({'Faction': i['Name'], 'Influence' : i['Influence'], 'States': []})
    ...
    if entry['event'] == 'FSDJump':  # get factions at jump
        ...
        for i in entry['Factions']:
            if i['Name'] != "Pilots' Federation Local Branch":
                ...
                this.FactionStates['Factions'].append(
                    {'Faction': i['Name'], 'Influence': i['Influence'], 'States': []})
        ...
        if x >= 1:
            ...
            for i in range(0, z):
                ...
                inf = round(inf, 2)
            for i in range(0, z):
                state = this.FactionStates['Factions'][i]['States'] 
                this.TodayData[x + 1][0]['Factions'].append({'Faction': this.FactionNames[i], 'INF': inf, 'State': state, 'MissionPoints': 0, 'MissionFailed': 0,
                                                             'TradeProfit': 0, 'Bounties': 0, 'Bonds': 0, 'CartData': 0, 'Murders': 0})
        else:
            this.TodayData = {1: [{'System': entry['StarSystem'], 'SystemAddress': entry['SystemAddress'], 'Factions': []}]}
            z = len(this.FactionNames)
            ...
            for i in range(0, z):
                state = this.FactionStates['Factions'][i]['States'] 
                this.TodayData[1][0]['Factions'].append({'Faction': this.FactionNames[i], 'INF': inf, 'State': state, 'MissionPoints': 0, 'MissionFailed': 0,
                                                         'TradeProfit': 0, 'Bounties': 0, 'Bonds': 0, 'CartData': 0, 'Murders': 0})
        sheet_insert_new_system(x+1)
